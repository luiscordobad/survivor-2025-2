...
function Login() {
  ...
  const applyNewPassword = async (e) => {
    ...
    try {
      ...
      if (error) throw error;
      setResetInfo('Contraseña actualizada. Ya puedes entrar.');
...
function AppAuthed({ session }) {
  ...
  const TeamBadge = ({ id }) => {
    ...
    return (
      <span className="inline-flex items-center gap-2">
        {t.logo_url ? <img src={t.logo_url} alt={id} className="h-5 w-5 rounded-full" /> : null}
  ...
  const loadLeaguePicks = async (w) => {
    const { data: pks } = await supabase.from('picks')
      .select('user_id, team_id, result, auto_pick, updated_at').eq('week', w);
    const arr = pks || []; setLeaguePicks(arr);
    const ids = [...new Set(arr.map(x=>x.user_id))];
    if (ids.length) {
      const { data: profs } = await supabase.from('profiles').select('id,display_name').in('id', ids);
      const map = {}; (profs||[]).forEach(p=>{ map[p.id]=p.display_name; }); setUserNames(map);
    } else { setUserNames({}); }
    const { data: total } = await supabase.from('standings').select('user_id');
    const counts = {};
    arr.forEach(x=>{ if(x.team_id) counts[x.team_id]=(counts[x.team_id]||0)+1; });
    const list = Object.entries(counts).map(([team_id,count])=>({
      team_id, count, pct: total?.length ? Math.round((count*100)/total.length) : 0
  ...
  const load = async () => {
    ...
    try { await ensureLeagueMembership(session.user.id); } catch {}
  ...
  useEffect(()=>{ load(); }, []);
  useEffect(()=>{ loadGames(week); loadLeaguePicks(week); setPopWeek(week); }, [week]);
  ...
  const gamesFiltered = useMemo(()=>{
    ...
    const match = (teamId)=>{
      const t=teamsMap[teamId]; return teamId.toLowerCase().includes(q) || (t?.name||'').toLowerCase().includes(q);
    };
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {g.status==='in_progress' && (
                        <div className="mt-1 text-xs text-amber-800">{g.period?`Q${g.period}`:''} {g.clock||''}</div>
                      )}
                    </div>
        ...
        <section className="mt-6 grid md:grid-cols-2 gap-4">
          ...
          <div className="p-4 border rounded-2xl bg-white shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <select className="border p-1 rounded" value={popWeek} onChange={e=>{ setPopWeek(Number(e.target.value)); loadLeaguePicks(Number(e.target.value)); }}>
                {Array.from({length:18},(_,i)=>i+1).map(w=><option key={w} value={w}>W{w}</option>)}
              </select>
            </div>
